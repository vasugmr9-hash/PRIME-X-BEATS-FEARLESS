...
async def download_media(video_id: str, is_video: bool = False) -> str:
    """Download media using the same yt-dlp path as the known-working bot.

    The previous replacement added several sequential extraction clients and
    ffmpeg re-encoding steps. That made /vplay wait a long time when YouTube
    extraction failed. For stability, keep this path simple: one yt-dlp
    extraction using web_embedded/web, then hand the resulting MP4 directly
    to PyTgCalls.
    """
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)

    # Reuse a completed file. Keep video and audio caches separate.
    if is_video:
    ...
    if os.path.exists(final_path) and os.path.getsize(final_path) > 10000:
        if await _probe_media(final_path, is_video):
            return final_path
        try:
    ...
    try:
        ...
        opts = {
            ...
            # Do not force the old web_embedded/web clients.
            # YouTube changed its player-response/PO-token behavior; yt-dlp's
            # current default client selection is more reliable for /vplay.
        }

        # Use the same stable format policy as the known-working /vplay path.
        # 480p is intentional here: it is the tested Telegram VC path and
        # avoids the high-bitrate/codec combinations that caused stalls.
        if is_video:
            opts.update({
        ...
        print(f"[yt-dlp] downloading {'video' if is_video else 'audio'}: {video_id}")
        info = await asyncio.wait_for(
            loop.run_in_executor(None, _extract),
            timeout=35,
        )

        candidates = [x for x in glob.glob(f"{base_path}.*") if not x.endswith(".part")]
        if not candidates:
            raise RuntimeError("yt-dlp completed without producing a media file")

        # Prefer MP4 for video and MP3/M4A for audio when available.
        if is_video:
            candidates.sort(key=lambda x: (not x.lower().endswith(".mp4"), -os.path.getsize(x)))
        else:
            candidates.sort(key=lambda x: (not x.lower().endswith((".mp3", ".m4a", ".webm")), -os.path.getsize(x)))

        produced = candidates[0]
        if os.path.abspath(produced) != os.path.abspath(final_path):
            os.replace(produced, final_path)

        if not await _probe_media(final_path, is_video):
            raise RuntimeError("yt-dlp produced an invalid media file")

        print(f"[yt-dlp] ready: {final_path}")
        return final_path
