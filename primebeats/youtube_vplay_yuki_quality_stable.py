...
async def download_media(video_id: str, is_video: bool = False) -> str:
    ...
    IMPORTANT:
    - The video branch is used by /vplay and uses the same Yuki API path
      that previously produced a working MP4 in this bot.
    - The audio branch is left exactly on the existing yt-dlp path.
    This keeps /play unchanged while fixing /vplay only.
    """
    ...
    if is_video:
        ...
        # This is the previously working /vplay download route.
            ...
            try:
                ...
                async with session.get(
                    ...
                    try:
                        ...
                        # Normalize the downloaded video into a Telegram-friendly
                        # H.264/AAC MP4. This keeps the source at 720p when Yuki
                        # provides it, while avoiding codec/bitrate combinations
                        # that can cause stutter or glitches in VC playback.
                        if quality >= 720 and shutil.which("ffmpeg"):
                            normalized = os.path.join(
                                DOWNLOAD_DIR, f"{video_id}.vplay.normalized.mp4"
                            )
                            try:
                                proc = await asyncio.create_subprocess_exec(
                                    "ffmpeg", "-y", "-i", tmp_path,
                                ...
                                if proc.returncode == 0 and await _probe_media(normalized, True):
                                    os.replace(normalized, final_path)
                                    print(
                                        f"[Yuki API] vplay ready: {final_path} "
                                        f"(quality={quality}, normalized=720p/H264)"
                                    )
                                    return final_path
                                raise RuntimeError(
        ...
        raise RuntimeError(
            f"Yuki API video download failed: {last_error}"
        )

    # AUDIO /play path below is intentionally unchanged from Fix 3.
    ...
    try:
        ...
        for client in clients:
            ...
            print(
                f"[yt-dlp] play attempt client={client} "
                f"type=audio: {video_id}"
            )
            try:
                ...
                if await _probe_media(final_path, False):
                    print(
                        f"[yt-dlp] play ready client={client}: {final_path}"
                    )
                    return final_path
