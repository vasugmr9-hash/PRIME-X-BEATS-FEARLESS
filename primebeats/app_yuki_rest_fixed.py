...
class PrimeBeats:
    ...
    def _make_media(self, track, video: bool, effect: str, speed: float = 1.0, start_at: float = 0.0):
        # Vibe Fearless-style PyTgCalls pipeline. Direct Yuki media is passed to
        # PyTgCalls; FFmpeg is used only when seeking/effects/speed actually need it.
        ff = self._audio_filter(effect, speed)
        ffmpeg_parts = []
        audio_start = ["-thread_queue_size", "4096", "-threads", "4"]
        if start_at > 0:
            audio_start.extend(["-ss", str(int(max(0.0, float(start_at))))])
        ffmpeg_parts = ["--audio", "---start", " ".join(audio_start)]
        if ff:
            ffmpeg_parts.extend(["---mid", "-af", ff])
        if video:
            video_start = ["-thread_queue_size", "4096", "-threads", "4"]
            if start_at > 0:
                video_start.extend(["-ss", str(int(max(0.0, float(start_at))))])
            ffmpeg_parts.extend(["--video", "---start", " ".join(video_start)])
        ffmpeg_params = " ".join(ffmpeg_parts)
        if video:
            return MediaStream(
                track.stream_url,
                AudioQuality.HIGH,
    ...
    async def _restart_current(self,chat_id,position:float|None=None):
        ...
        self._cancel_end_watchdog(chat_id)
        fresh=await asyncio.wait_for(resolve(target.webpage_url,target.requested_by,getattr(target,"video",p.video)),timeout=60)
            ...
            if data=="searchclose":
                ...
                try:
                    ...
                    if not entry.get("id") and not entry.get("link"):
                        raise ValueError("search result has no playable YouTube id")
                    await q.answer("⏳ Loading song…")
                    # Use the exact cached search result. This avoids a second
                    # metadata lookup and, importantly, never calls Yuki /details.
                    ...
                    await self._announce_track(chat,track,p)
                    with suppress(Exception):
