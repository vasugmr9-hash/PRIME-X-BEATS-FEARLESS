...
async def _ytdlp_resolve(link_or_query: str, video: bool = False) -> dict:
    ...
    def _extract():
        opts = {
            ...
            # Let current yt-dlp choose its maintained YouTube clients.
            # Forcing android/web here is unreliable because YouTube now
            # applies different PO-token/player-response requirements.
        }
        if cookie_file and os.path.isfile(cookie_file):
            opts["cookiefile"] = cookie_file
        if video:
            opts["format"] = "best[height<=720][ext=mp4][vcodec!=none][acodec!=none]/best[height<=720][vcodec!=none][acodec!=none]/best"
        else:
