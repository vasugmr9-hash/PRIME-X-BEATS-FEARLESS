...
async def _search_one(query: str) -> dict | None:
    ...
    vercel_results = await _search_vercel(
        query,
        limit=1,
    )
...
async def download_media(
    ...
    if not API_KEY or API_KEY == "YOUR_API_KEY":
        print(
            "[Yuki API] Error: API_KEY is not set "
            "in environment or config."
        )

        raise RuntimeError(
            "API_KEY missing. Please set MEOW_API_KEY "
            "in .env or get from @MeowApiRobot"
        )
        ...
        async with session.get(
            ...
            if resp.status == 200:
                ...
                if (
                    ...
                    and os.path.getsize(tmp_path) > 10000
                ):
                    os.replace(
                        tmp_path,
                        out_path,
                ...
                raise RuntimeError(
                    "Invalid API_KEY. "
                    "Please get a valid key from @MeowApiRobot"
                )

    except Exception as e:

        print(
            f"[Yuki API] Stream error for "
            f"{video_id}: {e}"
        )
    ...
    raise RuntimeError(
        f"Yuki API download failed for {video_id}"
    )
...
class YouTubeAPI:
    ...
    async def url(
        ...
        text = (
            ...
            if hasattr(message_or_text, "text")
            else str(message_or_text)
        )
    ...
    async def details(
        ...
        if not r:
            raise RuntimeError(
                f"No track details found for: {link}"
            )
...
async def resolve(
    ...
    info = await _ytdlp_resolve(
        link_or_query,
        video=bool(video),
    )

    track = _make_prime_track(
        info,
        requested_by,
    )
...
async def resolve_playlist(
    ...
    urls = await YouTubeAPI().playlist(
        link,
        int(limit),
        0,
    )
