...
#
#   PLAYBACK:
#       1. yt-dlp local original-quality audio (no MP3 conversion)
#       2. yt-dlp local best-stable video download (MP4, up to 720p)
#
# Yuki is intentionally NOT used for playback.
#
...
def _ytdlp_search_sync(query: str, limit: int) -> list[dict]:
    ...
    # Use the same hardened yt-dlp defaults as playback: writable cookie copy,
    # current YouTube EJS support, and optional bgutil PO-token provider.
...
async def search_results(query: str, limit: int = 8) -> list[dict]:
    """Fast, resilient YouTube search with no Yuki dependency.

    The providers run concurrently instead of sequentially. A slow/dead
    provider therefore cannot block /search for the full fallback chain.
    Yuki is deliberately excluded from search and playback.
    """
    ...
    # For a direct YouTube URL/ID, yt-dlp is the authoritative resolver.
    # For normal text search, race independent fast providers.
    if _extract_video_id(query):
        providers = [("yt-dlp", _search_ytdlp)]
    else:
        providers = [
            ("py-yt-search", _search_pyyt),
            ("yt-dlp", _search_ytdlp),
            ("vercel", _search_vercel),
        ]

    async def run_provider(name, provider):
        try:
            rows = await asyncio.wait_for(provider(query, limit), timeout=7.5)
            if rows:
                print(f"[YouTube search] {name} OK: {query!r} results={len(rows)}")
                return rows
            print(f"[YouTube search] {name} returned no results: {query!r}")
        except asyncio.TimeoutError:
            print(f"[YouTube search] {name} timed out: {query!r}")
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            print(f"[YouTube search] {name} failed: {exc}")
        return []

    tasks = [asyncio.create_task(run_provider(name, provider)) for name, provider in providers]
    try:
        # Return the first non-empty result set. This makes /search fast even
        # when one backend is having a bad day.
        pending = set(tasks)
        while pending:
            ...
            for task in done:
                ...
                if rows:
                    ...
                    if clean:
                        ...
                        _SEARCH_CACHE[cache_key] = (loop.time(), clean[:limit])
                        return clean[:limit]
    finally:
        for task in tasks:
            if not task.done():
...
async def _yuki_direct_stream(video_id: str, video: bool) -> str:
    """Build Yuki's key-authenticated direct stream URL."""
    vid = str(video_id or "").strip()
    if not re.fullmatch(r"[A-Za-z0-9_-]{11}", vid):
        raise RuntimeError("Invalid YouTube video ID")

    key = (
        os.environ.get("YUKI_API_KEY", "").strip()
        or os.environ.get("MEOW_API_KEY", "").strip()
    )
    if not key:
        raise RuntimeError("Yuki API key is not configured")
...
def _writable_youtube_cookie_file() -> str | None:
    """Return a writable private copy of the configured YouTube cookies.

    Always copy the source to /tmp when it is not already that exact path.
    This is intentional: Render Secret Files such as /etc/secrets/cookies.txt
    are read-only, and yt-dlp can update its cookie jar during a download.
    The original secret is never modified.
    """
    import shutil
    ...
    try:
        ...
        if (
            not os.path.exists(writable)
            or os.path.getmtime(writable) < source_mtime
            or os.path.getsize(writable) != source_size
        ):
...
def _yt_dlp_common_opts() -> dict:
    ...
    cookie_file = _writable_youtube_cookie_file()
    if cookie_file:
        opts["cookiefile"] = cookie_file
        print(f"[yt-dlp] using writable cookies: {cookie_file}")
    return opts

def _ytdlp_audio_url_sync(video_id: str) -> str:
    """Download the original best YouTube audio locally and return its path.

    No MP3 conversion is performed.  ``bestaudio`` is kept in the codec/container
    selected by yt-dlp so we do not throw away source quality before FFmpeg and
    NTgCalls send the audio into Telegram's voice-chat codec.
    """
    import glob
    import os
    import yt_dlp
    ...
    final_base = os.path.abspath(
        os.path.join(DOWNLOAD_DIR, f"{video_id}.ytdlp.audio")
    )

    # Reuse a previously downloaded original-quality source.
    for candidate in glob.glob(final_base + ".*"):
        if (
            os.path.isfile(candidate)
            and os.path.getsize(candidate) > 10000
            and not candidate.endswith((".part", ".ytdl", ".ytdl.part"))
        ):
    ...
    for name, clients in attempts:
        attempt_base = os.path.abspath(
            os.path.join(DOWNLOAD_DIR, f"{video_id}.ytdlp.audio.{name}")
        )
        opts = _yt_dlp_common_opts()
        opts.pop("skip_download", None)
        opts.update({
            # Keep the best source audio exactly as downloaded.  Do not convert
            # Opus/AAC/etc. to MP3 because that would be an unnecessary lossy step.
            "format": "bestaudio/best",
            "outtmpl": attempt_base + ".%(ext)s",
            "retries": 3,
            "fragment_retries": 3,
            "extractor_retries": 3,
            "socket_timeout": 30,
        ...
        try:
            print(f"[yt-dlp] BEST audio download client={name} video_id={video_id}")
            with yt_dlp.YoutubeDL(opts) as ydl:
                ydl.download([target])

            media_candidates = []
            for candidate in glob.glob(attempt_base + ".*"):
                if (
                    os.path.isfile(candidate)
                    and os.path.getsize(candidate) > 10000
                    and not candidate.endswith((".part", ".ytdl", ".ytdl.part"))
                ):
                    media_candidates.append(os.path.abspath(candidate))

            if not media_candidates:
                raise RuntimeError("yt-dlp did not create a playable local audio file")

            # Move the selected original source to a stable cache name.  Keep its
            # real extension (webm/m4a/opus/etc.) so FFmpeg can read it natively.
            ...
            print(
                f"[yt-dlp] BEST audio ready client={name}: {final_path} "
                f"size={os.path.getsize(final_path)}"
            )
            return final_path
        except Exception as exc:
            errors.append(f"{name}: {str(exc)[-500:]}")
            print(f"[yt-dlp] BEST audio client={name} failed: {exc}")
        finally:
            # Only remove temporary attempt files.  Never remove the canonical
            # cached source returned above.
            for leftover in glob.glob(attempt_base + ".*"):
                try:
                    os.remove(leftover)
                except OSError:
                    pass

    raise RuntimeError("yt-dlp best audio download failed | " + " || ".join(errors))
...
def _ytdlp_video_file_sync(video_id: str) -> str:
    ...
    for name, clients in attempts:
        ...
        try:
            ...
            candidates = (
                out_path,
                f"{out_path}.mp4",
                os.path.join(DOWNLOAD_DIR, f"{video_id}.fallback.mp4.mp4"),
            )
    ...
    raise RuntimeError("yt-dlp video extraction failed | " + " || ".join(errors))
...
async def _get_stream(video_id: str, video: bool) -> str:
    """Resolve playback exclusively through yt-dlp.

    Yuki is intentionally NOT used for playback. Audio is downloaded locally
    in its best available source format, and video is downloaded locally as
    best available quality up to 720p, so PyTgCalls/FFmpeg receives a stable
    filesystem source instead of an expiring remote URL.
    """
    stream = await _fallback_stream(video_id, video)
    print(
        f"[Playback] yt-dlp PRIMARY OK: id={video_id} "
        f"type={'video' if video else 'audio'} source=local"
    )
    return stream
...
async def download_song(link: str) -> str:
    """Download a song with yt-dlp only and return its local file path."""
    ...
    return await asyncio.to_thread(_ytdlp_audio_url_sync, vid)
