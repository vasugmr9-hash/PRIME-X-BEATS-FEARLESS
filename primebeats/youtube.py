...
def _cookie_file() -> str | None:
    """Return the first usable YouTube cookie file path."""
    candidates = [
...
def _yt_dlp_common_opts() -> dict:
    """Common yt-dlp options for Render/YouTube playback fallback."""
    opts = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
        ...
        "extractor_args": {
            "youtubepot-bgutilhttp": {
                "base_url": os.environ.get(
                    ...
                    os.environ.get("BGUTIL_POT_BASE_URL", "http://127.0.0.1:4416"),
                )
            }
        },
    }
    ...
    # yt-dlp's current YouTube extractor may require a JS runtime/EJS.
...
def _yt_dlp_download_sync(video_id: str, is_video: bool, out_dir: str) -> str:
    """Download a playable local file when the Yuki provider is unavailable."""
    import yt_dlp

    target = f"https://www.youtube.com/watch?v={video_id}"
    cookie_file = _cookie_file()

    base = _yt_dlp_common_opts()
    base.update({
        "outtmpl": os.path.join(out_dir, f"{video_id}.ytfallback.%(ext)s"),
        "overwrites": True,
    })
    ...
    # Try several YouTube client profiles. Cookies are used whenever the
    # Render Secret File is present.
    client_profiles = [
        None,
        ["mweb"],
        ["web_creator"],
    ...
    for clients in client_profiles:
        ...
        try:
            ...
            candidates = sorted(
                Path(out_dir).glob(f"{video_id}.ytfallback.*"),
            ...
            last_error = str(exc)
            print(
                f"[yt-dlp fallback] failed video_id={video_id} "
                f"client={clients or 'default'}: {exc}"
            )

    raise RuntimeError(
        f"yt-dlp fallback failed for {video_id}: {last_error or 'unknown error'}"
    )

async def download_media(video_id: str, is_video: bool = False) -> str:
    if not API_KEY or API_KEY == "YOUR_API_KEY":
        print("[Yuki API] API_KEY is not set; using yt-dlp fallback.")
    ...
    # First try the configured Yuki API exactly as before.
                    ...
                    if resp.status == 200:
                        ...
                        if os.path.exists(tmp_path) and os.path.getsize(tmp_path) > 10000:
                            ...
                            if await _probe_media(out_path, is_video):
                                ...
                                return out_path
                            last_error = (
                                "provider returned a file without the requested media stream"
                            )
                    elif resp.status == 401:
                        raise RuntimeError(
                            "Invalid API_KEY. Please get a valid key from @MeowApiRobot"
                        )
                    else:
                        last_error = f"HTTP {resp.status}"
                        print(
                            f"[Yuki API] HTTP {resp.status} for "
                            f"{req_type} video_id={video_id}"
                        )
    ...
    # Yuki can return transient/provider-side 5xx responses (including 530).
    # Fall back to local yt-dlp so /play is not completely dependent on Yuki.
    print(
        f"[Playback] Yuki unavailable for {video_id} "
        f"({last_error or 'API key missing'}); trying yt-dlp fallback."
    )
    ...
    try:
        fallback_path = await loop.run_in_executor(
            None,
            _yt_dlp_download_sync,
            video_id,
            bool(is_video),
            DOWNLOAD_DIR,
        )
        if await _probe_media(fallback_path, is_video):
            print(f"[yt-dlp fallback] ready: {fallback_path}")
            return fallback_path
        raise RuntimeError("yt-dlp produced a file without the requested media stream")
    except Exception as exc:
        raise RuntimeError(
            f"Yuki API {req_type} failed "
            f"({last_error or 'unavailable'}); yt-dlp fallback also failed: {exc}"
        ) from exc
