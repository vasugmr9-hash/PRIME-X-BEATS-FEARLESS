...
async def download_media(video_id: str, is_video: bool = False) -> str:
    """VPLAY-only YouTube downloader.

    Uses a short client fallback chain because YouTube is currently changing
    player-response/PO-token requirements. The rest of the PRIME x BEATS API
    is intentionally left unchanged.
    """
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)

    final_path = os.path.join(
        DOWNLOAD_DIR, f"{video_id}.vplay.mp4" if is_video else f"{video_id}.play.mp3"
    )
    ...
    try:
        ...
        # Keep each attempt short. android_vr commonly exposes the pre-merged
        # format 18 without a GVS PO token; web_safari can expose HLS formats.
        # web_embedded is retained as a fallback for embeddable videos.
        ...
        for client in clients:
            ...
            if is_video:
                # Prefer <=480p merged/separate streams, with format 18 as the
                # important no-PO-token fallback for the android_vr client.
                opts.update({
            ...
            print(f"[yt-dlp] vplay attempt client={client} type={'video' if is_video else 'audio'}: {video_id}")
            try:
                ...
                if await _probe_media(final_path, is_video):
                    print(f"[yt-dlp] vplay ready client={client}: {final_path}")
                    return final_path
                raise RuntimeError("produced media failed ffprobe validation")

            except Exception as e:
                last_error = e
                print(f"[yt-dlp] vplay client={client} failed: {e}")
            finally:
        ...
        raise RuntimeError(f"all YouTube clients failed; last error: {last_error}")

    except Exception as e:
        print(f"[yt-dlp] vplay failed for {video_id}: {e}")
