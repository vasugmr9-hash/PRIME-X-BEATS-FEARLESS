...
async def _run_ytdlp_video(video_id: str) -> str:
    """Download high-quality video and normalize it for stable PyTgCalls playback."""
    try:
        import yt_dlp
    ...
    final_path = os.path.join(DOWNLOAD_DIR, f"{video_id}.vplay720.h264.mp4")

    if os.path.exists(final_path) and os.path.getsize(final_path) > 10000:
        if await _probe_media(final_path, True):
            print(f"[vplay] cached H264 video: {final_path}")
            return final_path
    ...
    last_error = None
    formats = [
        "bv*[height<=720][vcodec^=avc1][ext=mp4]+ba[acodec^=mp4a][ext=m4a]/bv*[height<=720][vcodec^=avc1][ext=mp4]+ba[ext=m4a]/bv*[height<=720][vcodec^=avc1]+ba/b[height<=720][vcodec^=avc1][ext=mp4]/b[height<=720][ext=mp4]/bv*[height<=720]+ba/b[height<=720]",
        "bv*[height<=720]+ba/b[height<=720]",
        "b[height<=720][ext=mp4]/b[height<=720]",
    ]
    clients = [("android_vr", ["android_vr"]), ("tv", ["tv"]), ("web_embedded", ["web_embedded"]), ("web_safari", ["web_safari"]), ("default", None)]

    for client_name, player_clients in clients:
        for fmt in formats:
            base = os.path.join(DOWNLOAD_DIR, f"{video_id}.vplay_source.{random.randint(10000,99999)}")
            def _download(fmt=fmt, player_clients=player_clients, base=base):
                opts = {
                    "format": fmt, "outtmpl": base + ".%(ext)s", "merge_output_format": "mp4",
                    "noplaylist": True, "quiet": True, "no_warnings": True,
                    "retries": 3, "fragment_retries": 5, "extractor_retries": 3,
                    "socket_timeout": 30, "concurrent_fragment_downloads": 2,
                    "overwrites": True, "restrictfilenames": True,
                }
                if player_clients:
                ...
                for candidate in (requested, os.path.splitext(requested)[0]+".mp4", base+".mp4", base+".mkv", base+".webm"):
                    if os.path.exists(candidate) and os.path.getsize(candidate) > 10000:
                        return candidate
                raise RuntimeError("yt-dlp produced no media file")
            try:
                print(f"[vplay] yt-dlp client={client_name} format={fmt[:100]}")
                source = await asyncio.wait_for(loop.run_in_executor(None, _download), timeout=150)
                ffmpeg = await asyncio.create_subprocess_exec(
                    "ffmpeg", "-y", "-i", source, "-map", "0:v:0", "-map", "0:a:0?",
                    "-vf", "scale=trunc(min(1280,iw)/2)*2:trunc(min(720,ih*1280/iw)/2)*2,fps=30",
                ...
                _, err = await asyncio.wait_for(ffmpeg.communicate(), timeout=240)
                if ffmpeg.returncode == 0 and os.path.exists(final_path) and os.path.getsize(final_path) > 10000 and await _probe_media(final_path, True):
                    print(f"[vplay] H264/AAC ready: {final_path}")
                    try: os.remove(source)
                    except OSError: pass
                    return final_path
                raise RuntimeError(f"FFmpeg normalization failed: {err.decode(errors='ignore')[-1500:]}")
            except Exception as exc:
                last_error = str(exc)
                print(f"[vplay] failed client={client_name}: {type(exc).__name__}: {exc}")
            finally:
                for candidate in (base, base+".mp4", base+".mkv", base+".webm", base+".part", base+".ytdl"):
                    try:
                        if os.path.exists(candidate): os.remove(candidate)
                    except OSError: pass
    raise RuntimeError(f"yt-dlp video download failed: {last_error or 'unknown error'}")


async def download_media(video_id: str, is_video: bool = False) -> str:
    """Use yt-dlp for high-quality video; keep the working Yuki audio path."""
    ...
    if is_video:
        try:
            return await _run_ytdlp_video(video_id)
        except Exception as ytdlp_error:
            print(f"[vplay] yt-dlp primary failed: {ytdlp_error}")
            print("[vplay] Falling back to Yuki API.")
