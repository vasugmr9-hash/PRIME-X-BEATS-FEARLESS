...
class PrimeBeats:
    ...
    async def stream(self,chat_id,track,video=False,effect=None,start_at:float=0.0):
        ...
        if video:
            media=MediaStream(fresh.stream_url,AudioQuality.HIGH,VideoQuality.SD_480p,**kwargs)
        else:
    ...
    def _register(self):
        ...
        async def vplay(_,m):
            if m.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP, ChatType.CHANNEL):await m.reply_text("❌ Use /vplay in a group/channel.");return
            if not await self.require_admin(m):return
            q=m.text.split(maxsplit=1)[1] if len(m.text.split(maxsplit=1))>1 else ""
        ...
        async def callbacks(_,q):
            ...
            if data=="searchclose":
                ...
                try:
                    ...
                    url=(entry.get("webpage_url") or entry.get("url") or entry.get("link"))
                    if not url:
                        vid=entry.get("id")
                        if vid:
