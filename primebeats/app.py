...
class PrimeBeats:
    ...
    @staticmethod
    def _position(p) -> float:
        """Return the real playback position, including paused state."""
        if not p.current or not p.started_at:
            return 0.0
        now = getattr(p, "paused_at", 0.0) or time.monotonic()
        pos = max(0.0, now - p.started_at)
        if p.current.duration:
            pos = min(pos, float(p.current.duration))
        return pos
    ...
    def _arm_end_watchdog(self, chat_id, track):
        ...
        async def watch():
            try:
                # PyTgCalls normally emits stream_end. This watchdog is a
                # pause-aware safety net when the native event is delayed/missed.
                while True:
                    ...
                    if p.current is not track:
                        return
                    if p.paused:
                        continue
                    if self._position(p) >= max(0.0, float(track.duration)-0.25):
                        await asyncio.sleep(1.0)
                        if chat_id in self._transitioning or time.monotonic() < self._manual_stop_until.get(chat_id,0.0):
                            return
                        p=self.store.get(chat_id)
    ...
    async def stream(self,chat_id,track,video=False,effect=None,start_at:float=0.0,refresh=False):
        ...
        old_state=(p.current,p.paused,getattr(p,"paused_at",0.0),p.muted,p.video,p.effect,p.started_at)
        fresh=track
        if refresh or not getattr(track,"stream_url",None):
            fresh=await asyncio.wait_for(resolve(track.webpage_url,track.requested_by,video),timeout=120)
        ...
        try:
            await asyncio.wait_for(maybe(self.calls.play(chat_id,media)), timeout=60)
            ...
            p.current,p.paused,p.paused_at,p.muted,p.video,p.effect,p.started_at=old_state
            raise
        ...
        p.current=track; p.paused=False; p.paused_at=0.0; p.muted=False; p.video=bool(video); p.effect=effect_key
    ...
    async def _restart_current(self,chat_id,position:float|None=None):
        ...
        self._cancel_end_watchdog(chat_id)
        fresh=await asyncio.wait_for(resolve(target.webpage_url,target.requested_by,getattr(target,"video",p.video)),timeout=120)
    ...
    async def handle_play(self,m,query,video=False):
        ...
        try:
            track=await asyncio.wait_for(resolve(query,self._requester_name(m.from_user),video),timeout=180)
            track.video=bool(video)
            ...
            async with p.lock:
                ...
                await asyncio.wait_for(self.stream(m.chat.id,track,video,p.effect), timeout=90)
            await msg.delete()
    ...
    async def _natural_end(self, chat_id, expected=None):
        ...
        async with p.lock:
            ...
            p.remember(finished)
            p.current=None
            p.paused=False; p.paused_at=0.0
    ...
    def _register(self):
        ...
        async def effect(_,m):
            ...
            if p.current:
                try:
                    pos=self._position(p); await self._restart_current(m.chat.id,pos)
                except Exception:
                    p.effect=old_effect
        ...
        async def control(m,action):
            ...
            try:
                if action=="pause":
                    await maybe(self.calls.pause(m.chat.id))
                    if p.current and not p.paused:
                        p.paused=True; p.paused_at=time.monotonic()
                elif action=="resume":
                    await maybe(self.calls.resume(m.chat.id))
                    if p.current and p.paused:
                        now=time.monotonic(); p.started_at += max(0.0, now-getattr(p,"paused_at",now)); p.paused=False; p.paused_at=0.0
                elif action=="skip":
                    ...
                    # /stop is a full shutdown: cancel autoplay, clear queue, leave VC,
                    # and send one explicit stop message. Never render Player Ready here.
                    task=self._autoplay_tasks.pop(m.chat.id,None)
                    if task and not task.done():
                        task.cancel()
                        with suppress(asyncio.CancelledError): await task
                    stopped=p.current.title if p.current else "Nothing was playing"
                    queued=len(p.queue)
                    self._manual_stop_until[m.chat.id]=time.monotonic()+8.0
                    self._cancel_end_watchdog(m.chat.id)
                    p.clear(); p.current=None; p.autoplay=False; p.autoplay_topic=""; p.autoplay_seen.clear(); p.paused=False; p.paused_at=0.0
                    old_player=self._player_messages.pop(m.chat.id,None)
                    if old_player:
                        with suppress(Exception): await old_player.delete()
                    with suppress(Exception):
                        await asyncio.wait_for(maybe(self.calls.leave_call(m.chat.id)), timeout=5)
                    await m.reply_text(
                        f"⏹ <b>PLAYBACK STOPPED</b>\n\n🎵 {esc(stopped)}\n"
                        f"🧹 Queue cleared: <code>{queued}</code>\n♾️ Autoplay: <code>OFF</code>\n\n"
                        "▶️ Use <code>/play song</code> or <code>/vplay video</code> to start again.",
                        reply_markup=player_keyboard())
                    return
        ...
        @self.bot.on_message(filters.command(["nleft","left","remaining"]))
        async def nleft(_,m):
            if not await self.require_admin(m): return
            p=self.store.get(m.chat.id)
            if not p.current:
                await m.reply_text(
                    "⏳ <b>No song is currently playing.</b>\n\n📭 Queue: <code>{}</code>".format(len(p.queue)),
                    reply_markup=player_keyboard())
                return
            total=max(0,int(p.current.duration or 0))
            if not total:
                await m.reply_text("⏳ <b>Time left:</b> <code>LIVE / UNKNOWN</code>")
                return
            pos=min(total,int(self._position(p)))
            left=max(0,total-pos)
            mm,ss=divmod(left,60); tm,ts=divmod(total,60)
            state="⏸️ PAUSED" if p.paused else "▶️ PLAYING"
            await m.reply_text(
                f"⏳ <b>TIME LEFT</b>\n\n"
                f"🎵 <b>{esc(p.current.title)}</b>\n"
                f"{state}\n"
                f"📍 <code>{pos//60}:{pos%60:02d}</code> / <code>{tm}:{ts:02d}</code>\n"
                f"⏳ <b>Remaining:</b> <code>{mm}:{ss:02d}</code>\n"
        ...
        async def seek(_,m):
            ...
            try:
                current=self._position(p); target=max(0.0,current+secs)
    ...
    async def _control_callback(self,m,action,p):
        chat=m.chat.id
        if action=="pause":
            await maybe(self.calls.pause(chat))
            if p.current and not p.paused:
                p.paused=True; p.paused_at=time.monotonic()
        elif action=="resume":
            await maybe(self.calls.resume(chat))
            if p.current and p.paused:
                now=time.monotonic(); p.started_at += max(0.0, now-getattr(p,"paused_at",now)); p.paused=False; p.paused_at=0.0
        elif action=="skip":
            ...
            stopped=p.current.title if p.current else "Nothing was playing"
            queued=len(p.queue)
            self._manual_stop_until[chat]=time.monotonic()+8.0
            self._cancel_end_watchdog(chat)
            p.clear(); p.current=None; p.autoplay=False; p.autoplay_topic=""; p.autoplay_seen.clear(); p.paused=False; p.paused_at=0.0
            old_player=self._player_messages.pop(chat,None)
            if old_player:
                with suppress(Exception): await old_player.delete()
            with suppress(Exception):
