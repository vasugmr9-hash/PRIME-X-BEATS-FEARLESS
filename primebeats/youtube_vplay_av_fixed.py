...
async def _probe_media(path: str, require_video: bool) -> bool:
    """Verify that local media contains every stream PyTgCalls needs."""
    ...
    try:
        if require_video:
            # /vplay must contain BOTH video and audio. A video-only MP4 can
            # look valid to ffprobe but will produce a silent/failed VC stream.
            proc = await asyncio.create_subprocess_exec(
                "ffprobe", "-v", "error",
            ...
            if proc.returncode != 0:
                return False
            types = {x.strip() for x in out.decode(errors="ignore").splitlines() if x.strip()}
...
async def download_media(video_id: str, is_video: bool = False) -> str:
    """Download media for PRIME x BEATS.

    /vplay uses the working Yuki video endpoint directly. No local re-encode is
    performed because that made /vplay exceed the command timeout and appear
    stuck. Try the highest requested quality first, then fall back cleanly.
    The audio /play path remains unchanged.
    """
    os.makedirs(DOWNLOAD_DIR, exist_ok=True)

    if is_video:
        final_path = os.path.join(DOWNLOAD_DIR, f"{video_id}.vplay.mp4")
        if os.path.exists(final_path) and os.path.getsize(final_path) > 10000:
            if await _probe_media(final_path, True):
                return final_path
            try:
        ...
        if not API_KEY or API_KEY == "YOUR_API_KEY":
            raise RuntimeError("MEOW_API_KEY is not configured")

        session = await get_session()
        last_error = None

        # Prefer the confirmed-working 720p route first. If unavailable, try 1080p
        # and then smaller fallbacks.
        for quality in (720, 1080, 480, 360):
            tmp_path = os.path.join(
                DOWNLOAD_DIR, f"{video_id}.vplay.{quality}.tmp.mp4"
            )
            try:
                stream_url = (
                    f"{API_URL}/stream/{video_id}"
                    f"?key={urllib.parse.quote(API_KEY, safe='')}"
                    f"&type=video&quality={quality}"
                )
                print(
                    f"[Yuki API] vplay requesting video quality={quality} "
                    f"video_id={video_id}"
                )
                ...
                async with session.get(
                    ...
                    if resp.status != 200:
                        ...
                        print(f"[Yuki API] vplay quality={quality} failed: {last_error}")
                        continue
                ...
                if not await _probe_media(tmp_path, True):
                    last_error = f"quality={quality} returned invalid video"
                    print(f"[Yuki API] vplay failed: {last_error}")
                    continue

                os.replace(tmp_path, final_path)
                print(
                    f"[Yuki API] vplay ready: {final_path} "
                    f"(requested_quality={quality})"
                )
                return final_path

            except Exception as e:
                last_error = e
                print(f"[Yuki API] vplay quality={quality} failed: {e}")
            finally:
        ...
        raise RuntimeError(f"Yuki API video download failed: {last_error}")

    # AUDIO /play path below is intentionally unchanged.
    final_path = os.path.join(DOWNLOAD_DIR, f"{video_id}.play.mp3")

    if os.path.exists(final_path) and os.path.getsize(final_path) > 10000:
        if await _probe_media(final_path, False):
            return final_path
        try:
            os.remove(final_path)
        except OSError:
    ...
    work_id = f"{video_id}_{uuid.uuid4().hex[:8]}"
    base_path = os.path.join(DOWNLOAD_DIR, work_id)
    ...
    try:
        ...
        for client in clients:
            attempt_base = f"{base_path}_{client}"
            opts = {
                "quiet": True,
                "noprogress": True,
                "no_warnings": True,
                "outtmpl": f"{attempt_base}.%(ext)s",
                "geo_bypass": True,
            ...
            print(f"[yt-dlp] play attempt client={client} type=audio: {video_id}")
            try:
                await asyncio.wait_for(
                    loop.run_in_executor(None, _extract), timeout=18
                )
                candidates = [
                    x for x in glob.glob(f"{attempt_base}.*")
                    if not x.endswith(".part") and os.path.isfile(x)
                ]
                if not candidates:
                    raise RuntimeError("no media file produced")
                candidates.sort(key=lambda x: (
                    not x.lower().endswith((".mp3", ".m4a", ".webm")),
                    -os.path.getsize(x),
                ))
                ...
                if await _probe_media(final_path, False):
                    print(f"[yt-dlp] play ready client={client}: {final_path}")
                    return final_path
                raise RuntimeError("produced media failed ffprobe validation")
            except Exception as e:
                last_error = e
                print(f"[yt-dlp] play client={client} failed: {e}")
            finally:
                for leftover in glob.glob(f"{attempt_base}.*"):
                    try:
                        os.remove(leftover)
                    except OSError:
                        pass

        raise RuntimeError(f"all YouTube audio clients failed; last error: {last_error}")
    except Exception as e:
        print(f"[yt-dlp] play failed for {video_id}: {e}")
        raise RuntimeError(f"YouTube download failed: {e}") from e
    finally:
        for leftover in glob.glob(f"{base_path}.*"):
            try:
                os.remove(leftover)
            except OSError:
                pass
...
async def resolve(link_or_query: str, requested_by: str = "Unknown",
    ...
    # PyTgCalls receives local media as a filesystem path. Always pass an
    # absolute path; a relative "downloads/..." path is rejected by the
    # PRIME × BEATS stream-source validator.
