...
async def resolve_result(result: dict, requested_by: str = "Unknown",
                         video: bool = False):
    """Resolve one already-normalized search result without a second search."""
    if not isinstance(result, dict):
        raise TypeError("Search result must be a dictionary.")

    info = dict(result)
    vid = str(info.get("id") or "").strip()
    if not vid:
        vid = _extract_video_id(str(info.get("link") or "")) or ""
    if not vid:
        raise RuntimeError("Search result has no playable YouTube video id.")

    link = str(info.get("link") or f"https://www.youtube.com/watch?v={vid}").strip()
    info["id"] = vid
    info["link"] = link
    info.setdefault("title", f"YouTube Video ({vid})")
    info.setdefault("duration", "0:00")
    info.setdefault("thumbnails", [{"url": f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg"}])

    track = _make_prime_track(info, requested_by)
    media_path = await download_media(vid, is_video=bool(video))
    actual_duration = await _media_duration(media_path)
    metadata_duration = _seconds(info.get("duration"))
    resolved_duration = actual_duration or metadata_duration
    thumb = (info.get("thumbnails") or [{}])[0].get("url", "")
